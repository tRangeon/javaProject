...
public abstract class Personnage {
	...
	/**
	 * Methode autorise une transaction Si le montant est positif le personnage
	 * recoit de l'argent, s'il est négatif le personnage donne de l'argent
	 *
	 * @param montant
	 * @return autorisation Booleen qui indique si la transation est autorisee
	 *         ou non (true = autorisée; false = refusée)
	 */
	public boolean autorisationTransaction(int montant) {
		boolean autorisation;

		if (montant < 0) { // Cas ou on veut débiter de l'argent
			if (argent + montant < 0) { // Si les fonds sont inssufisants, alors
										// on refuse la transaction
				autorisation = false;
			} else { // sinon on l'autorise
				autorisation = true;
			}
		} else { // Cas ou on veut recevoir de l'argent, on autorise
					// systematiquement
			autorisation = true;
		}
		return autorisation;
	...
	/**
	 * Methode qui modifie l'argent du personnage Si le montant est positif le
	 * personnage recoit de l'argent, s'il est négatif il en perd Il est très
	 * fortement conseillé de vérifier avant que le personnage peut assumer
	 * cette transation en utilisant la methode
	 * autorisationTransaction(montant), sinon il risque d'avoir un niveau
	 * d'argent négatif
	 * 
	 * @param montant
	 */
	...
	/**
	 * Méthode qui effectue une transaction entre deux personnages Si le montant
	 * est positif, le personnage appelant la méthode va donner de l'argent au
	...
	public void transaction(Personnage deuxiemePersonnage, int montant) {
		...
		if (autorisationPersonnage1 && autorisationPersonnage2) { // Si la
																	// transaction
																	// est
																	// acceptée

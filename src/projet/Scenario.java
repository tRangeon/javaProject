...
public class Scenario {
	...
	/**
	 * Constructeur du Scénario, dans lequel on initalise toutes les instances
	 * du jeu
	 */
	public Scenario(Joueur joueur, MaitreDuJeu maitreDuJeu) {
		...
		// Creation des PNJ
		PNJ mathieu = new PNJ(1, "Mathieu", 20, 1, 500, new Inviter(maitreDuJeu, joueur, 3), new Repliques(true),
				joueur, maitreDuJeu);
		...
		PNJ mathilde = new PNJ(2, "Mathilde", 19, 2, 3000, new Inviter(maitreDuJeu, joueur, 2), new Repliques(true),
				joueur, maitreDuJeu);
		...
		PNJ agathe = new PNJ(3, "Agathe", 18, 2, 200, new PrendreArgent(joueur, 10, 1), new Repliques(false), joueur,
				maitreDuJeu);
		...
		agathe.getRepliques().ajouterReplique(3,
				"Tu aurais un peu de monaie ? Reviens me voir tout à l'heure pour que je te rembourse");
		agathe.getRepliques().ajouterReplique(2,
				"T'es un amour de m'avoir prêté de l'argent, tiens voilà ce que je te dois !");
		...
		PNJ helene = new PNJ(5, "Helene", 18, 2, 500, new FinirLeJeuPNJ(maitreDuJeu, 1), new Repliques(true), joueur,
				maitreDuJeu);
		helene.getRepliques().ajouterReplique(5, "Mon prince charmant, je t'ai enfin trouvé ! Emmène moi au bal !");

		PNJ gaetan = new PNJ(6, "Gaetan", 25, 1, 1000, new PrendreArgent(joueur, joueur.getArgent(), 0),
				new Repliques(true), joueur, maitreDuJeu);

		PNJ felix = new PNJ(7, "Félix", 21, 1, 1000, new DonnerArgent(joueur, 50, 0), new Repliques(true), joueur,
				maitreDuJeu);
		...
		PNJ camille = new PNJ(9, "Camille", 21, 2, 3000, new Discuter(joueur), new Repliques(true), joueur,
				maitreDuJeu);
		camille.getRepliques().getDiscussions().clear();
		camille.getRepliques().ajouterReplique(4,
				"Ca aurait été avec plaisir " + joueur.getNom() + ", mais j'ai déjà un cavalier pour le gala");

		PNJ seraphin = new PNJ(10, "Séraphin", 18, 1, 200, new Inviter(maitreDuJeu, joueur, 4), new Repliques(true),
				joueur, maitreDuJeu);

		PNJ thomas = new PNJ(11, "Thomas", 21, 1, 3000, new Inviter(maitreDuJeu, joueur, 6), new Repliques(true),
				joueur, maitreDuJeu);
		thomas.getRepliques().getInvitations().clear();
		thomas.getRepliques().getInvitations().add(
				"Yo, pour 50 balles je t'emmène dans la caverne d'Ali Baba mon pote ! Tu payes la moitié maintenant et le reste sur place");
		...
		sujet.getParametresObjet()
				.setAffichageTexte(joueur.getNom() + " lit le sujet du partiel de Java de la semaine prochaine !");
		...
		whisky.getParametresObjet().setAffichageTexte(
				"C'est un " + (joueur.getAge() + 10) + " ans d'âge, soit 10 ans plus vieux que " + joueur.getNom());
